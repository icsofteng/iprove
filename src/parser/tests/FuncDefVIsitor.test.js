...
test("Visitor Test Func Def friends", ()=> {
    expect(parse("define friends(Person, Person):Bool")).toEqual({ast:[funcDef], identifiers: [], relations: [], types:['Person']})
  })
...
test("Visitor Test Func Def Dragon", ()=> {
    expect(parse("define Power(dragon, Dragon):int")).toEqual({ast:[funcDef2], identifiers: [], relations: [], types:['Dragon']})
})
...
test("Visitor Test Func Def Happy", ()=> {
    expect(parse("define Happy(Dragon, Human):reward")).toEqual({ast:[funcDef3], identifiers: [], relations: [], types:['Reward' ,'Dragon', 'Human']})
})
